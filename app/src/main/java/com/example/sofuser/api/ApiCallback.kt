...
class ApiCallback<T: Any> constructor(private val exceptionRepository: ExceptionRepository,
    ...
    override fun onResponse(call: Call<T>, response: Response<T>) {
        if (!response.isSuccessful) {
            try {
                val exception = Gson().fromJson(response.errorBody()?.string(), BaseException::class.java)
                serviceCallBack.onError(exceptionRepository.generateShikomelException(exception))
